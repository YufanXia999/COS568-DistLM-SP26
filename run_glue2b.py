...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # ### NEW/MODIFIED FOR PART 2(b): Gradient Synchronization via all_reduce ###
            if args.local_rank != -1:
                for p in model.parameters():
                    if p.grad is None:
                        continue
                    
                    # 1. Sum the gradients across all nodes in-place
                    torch.distributed.all_reduce(p.grad.data, op=torch.distributed.ReduceOp.SUM)
                    
                    # 2. Divide by the total number of workers to get the average
